...
class UserController extends Controller
{
    ...
    public function mis_reservas(Request $request)
    {
        ...
        $reservas = Reserva::where('id_usuario', auth()->user()->id)
...
*/
    ...
    {
        $instalaciones = Instalacion::where('nombre', 'Like', '%' . $request->search . '%')
        ...
        foreach ($instalaciones as $instalacion) {
            $salida .=
                ...
                $instalacion->nombre .
                '</p>
                                        <p class="card-text" style="font-size: 12px"><small class="text-muted">(' .
    ...
    public function index_nuevo(Request $request)
    {
        ...
        $instalacion = Instalacion::where('slug', $request->slug_instalacion)->first();
        $eventos = Evento::where([['id_instalacion', $instalacion->id], ['insc_fecha_inicio', '<=', date('Y-m-d H:i:s')], ['insc_fecha_fin', '>=', date('Y-m-d H:i:s')]])
    ...
    public function ver_recibos(Request $request)
    {
        ...
        if (request()->slug_instalacion == 'villafranca-navidad') {
            $recibos_no_pagados = Recibo::where('id_usuario', $user->id)
