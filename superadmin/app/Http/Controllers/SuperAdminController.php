    ...
    public function update(Request $request, $id)
{
    ...
    if (!str_starts_with($url, 'https://gestioninstalacion.es/')) {
        if (str_starts_with($url, 'http://') || str_contains($url, 'gestioninstalacion.es')) {
            return redirect()->back()->withInput()->withErrors(['url' => 'La URL debe comenzar con https://gestioninstalacion.es/ o solo el nombre.']);
        }
    ...
    foreach ($request->all() as $key => $value) {
        if (preg_match('/^nombre_(\d+)$/', $key, $matches)) {
            ...
            $secondaryConnection->table('instalaciones')->where('id', $instalacionId)->update([
                'nombre' => $value,
    ...
    public function store(Request $request)
{
    ...
    // Validar y procesar la URL
    if (empty($url)) {
        return redirect()->back()->withInput()->withErrors(['url' => 'El campo URL no puede estar vacío.']);
    }
